...
for Participant_Num = 1:N_Participants
    ...
    if Participant_Num < 10
        filename = ['2023_10_26_S0' num2str(Participant_Num) 'POD1.csv'];
    else
        filename = ['2023_10_26_S' num2str(Participant_Num) 'POD2.csv'];
    end
    writematrix(SLA', filename);
    cd(Home)
end
%aligns all the axes ( i think)%
%linkaxes(ax)%
